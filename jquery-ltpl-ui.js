...
$(document).ready(function(){
	...
	$('[data-prTabs]').each(function(){
		var dP = $(this);
		var prTaBtabClick = dP.data('tab-click') || '';
		...
		$(this).find('[data-tab]').each(function(){
			...
			$(this).parents('[data-prTabs]').find('[data-tabContainer]').append($(this));
		});
		
		
		$(this).prepend('<ul class="pr-tab">'+li+'</ul>');
		li = '';
		counter = 0;
		//alert(defaultTabOptions.tabClick);
		if(defaultTabOptions.tabClick){
			$(this).find('[data-taber]').click(function(){
				var tab = $(this).attr('data-taber');
				dP.find('ul.pr-tab li').removeClass('active');
				dP.find('ul.pr-tab li[data-taber="'+tab+'"]').addClass('active');
				dP.find('[data-tab]').hide();
				dP.find('[data-tab="'+tab+'"]').show();
				//alert(tab);
			});
		}
		dP.find(defaultTabOptions.next).click(function(){
			if(dP.find('[data-taber].active').next().length > 0){
				var tab = dP.find('[data-taber].active').next().attr('data-taber');
				dP.find('ul.pr-tab li').removeClass('active');
				dP.find('ul.pr-tab li[data-taber="'+tab+'"]').addClass('active');
				dP.find('[data-tab]').hide();
				dP.find('[data-tab="'+tab+'"]').show();
		...
		dP.find(defaultTabOptions.prev).click(function(){
			if(dP.find('[data-taber].active').prev().length > 0){
				var tab = dP.find('[data-taber].active').prev().attr('data-taber');
				dP.find('ul.pr-tab li').removeClass('active');
				dP.find('ul.pr-tab li[data-taber="'+tab+'"]').addClass('active');
				dP.find('[data-tab]').hide();
				dP.find('[data-tab="'+tab+'"]').show();
...
// Image preview on browse
function imPreview(input, cls){
   cls = cls || '.imPreview';
   var reader;
   var x = [];
   var c = ($(cls+ ' .img').length <= 0)? 0: $(cls+ ' .img').length;
   //alert(c);
   if (input.files && input.files[0]){
       for(var i=0; i<input.files.length; i++){
           reader = new FileReader();
           reader.onload = function (e) {
               $(cls).append('<div class="img"><img src="'+e.target.result+'" width="100%" /><a class="close" data-index='+ c +'>X</a></div>');
               //$(cls).attr('src', e.target.result);
				$('.close').click(function(){
					$(this).parents('.img').remove();
					x.push($(this).data('index'));
					$('#remove_image').val(x.unique());
				});
				c++;
           }
   ...
   if($('#remove_image').length <= 0){
	$(input).parents('form').prepend('<input id="remove_image" type="hidden" name="removed" />');
	}
	$(input).hide(5).after($(input).clone());
}
